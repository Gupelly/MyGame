...
public class Character : Alive
{
    ...
    {
        if (!isDashing && !isJumpBack && isGrounded) rb.velocity = new Vector2(0, rb.velocity.y);
    ...
    void Update()
    {
        ...
        else
        {
            ...
            if (Input.GetButtonDown("Fire1") && !canNotDash)
            {
                isDashing = true;
                canNotDash = true;
                if (unlockDash) IsInvisable = true;
    ...
    private void Attack()
    {
        ...
        foreach (var bullet in bullets)
            bullet.GetComponent<Bullet>().ReceiveDamage();
        transform.position = new Vector2(rb.position.x - 0.1f * transform.localScale.x, rb.position.y);
    }
    ...
    {
        if (isDashing && unlockDash) return;
        isDashing = false;
        rb.gravityScale = 2.8f;
        rb.velocity = Vector2.zero;
        IsInvisable = true;
